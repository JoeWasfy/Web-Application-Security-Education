...
from .models import LabTopic, Challenge, Product, LoginPassword, FakeUser, Account5, AuthenticationCode, CustomUser
...
def update_user_points(request):
    user = request.user
    new_points = request.data.get('points', 0)
...
def my_account1(request):
    if request.user.is_authenticated:
        ...
        return redirect('lab_environment:login1')  # Redirect to login page for guest users
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            return redirect('lab_environment:my-account')
        else:
...
def logout_view(request):
    logout(request)
    return redirect('lab_environment:login1')
...
def failures(request):
    return render(request, 'static_pages/failures.html')

def auth_i(request):
    return render(request, 'static_pages/auth.html')

def crypto(request):
    return render(request, 'static_pages/crypto.html')

def security(request):
    return render(request, 'static_pages/security.html')

def ssrf(request):
    return render(request, 'static_pages/ssrf.html')

def xxe(request):
    return render(request, 'static_pages/xxe.html')
